...
def rnn_backward(dh, cache):
    ...
    “dh”包含每个时间步的单个损失函数产生的上游梯度，
    *而不是*时间步之间传递的梯度（您必须通过在循环中调用rnn_step_backward来计算）。
    Returns a tuple of:
...
def word_embedding_forward(x, W):
    ...
    我们假设一个V字的词汇表，将每个字赋给一个D维向量
    N个小批次 一个批次T长度的sequence x[i]为每个批次（句子，长度为T），每个位置x[i][j]对应一个字符，
    对应位置存放该字符的字符表中的位置0-V
    W为每个字符的向量表
    Inputs:
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    在整个数据序列上向前传递LSTM。我们假设一个输入序列由T个向量组成，每个向量的维数为D。
    LSTM使用隐藏大小H，我们处理一个包含N个序列的小批量。
    在运行LSTM forward之后，我们返回所有timesteps的隐藏状态。

    Note that the initial cell state is passed as input, but the initial cell
    state is set to zero. Also note that the cell state is not returned; it is
    an internal variable to the LSTM and is not accessed from outside.
    注意，初始单元状态作为输入传递，但初始单元状态设置为零。
    还请注意，单元状态没有返回；它是LSTM的内部变量，不能从外部访问。
    Inputs:
    ...
    for i in range(T):
        ht, ct, cache_cur = lstm_step_forward(x[:, i, :], ht, ct, Wx, Wh, b)
        h[:, i, :] = ht
...
def lstm_backward(dh, cache):
    ...
    for i in range(T - 1, -1, -1):
        dht += dh[:, i, :]
        dxt, dht, dct, dWxt, dWht, dbt = lstm_step_backward(dht, dct, cache_list[i])
        dx[:, i, :] = dxt
        dWx += dWxt
        dWh += dWht
        db += dbt
